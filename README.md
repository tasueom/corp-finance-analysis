...
- 🔍 **기업 검색**: 기업 이름으로 최근 10년치 재무상태표 데이터 조회
...
2. 기업 이름 입력 (예: "삼성전자")
3. 검색 버튼 클릭
4. 최근 10년치 재무상태표 데이터 확인
5. 필요시 "데이터 저장" 버튼으로 데이터베이스에 저장
...
3. **기업 이름**: 정확한 기업 이름을 입력해야 합니다. DART에 등록된 정확한 기업명을 사용하세요.
4. **데이터 조회 시간**: 10년치 데이터 조회 시 다소 시간이 걸릴 수 있습니다.
...
### 데이터 조회 실패
- 인터넷 연결 확인
- 기업 이름이 정확한지 확인 (예: "삼성전자" vs "삼성전자주식회사")
